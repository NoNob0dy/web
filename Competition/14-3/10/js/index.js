...
async function ajax({ url, method = "get", data, query: { currentPage, pageSize } }) {
    ...
    return axios.get(url).then(res => {
        ...
        let cnt = currentPage * pageSize;
        result.total = res.data.total;
        result.data = res.data.data.slice(cnt - pageSize, cnt > result.total ? result.total : cnt);
        return result;
    });
}
    ...
    renderPagination(indexArr) {
        ...
        for (let i = 0; i < indexArr.length; i++) {
            if (indexArr[i] == this.currentPage) {
                template = template.concat(`<li class='number active'>${indexArr[i]}</li>`);
            } else {
                if (indexArr[i] != indexArr[i - 1] + 1 && i != 0) {
                    template = template.concat(`<li class='number more'>...</li>`);
                }
                template = template.concat(`<li class='number'>${indexArr[i]}</li>`);
            }
        }
        this.root.innerHTML = `
    ...
    initEvents() {
        this.root.querySelector("#btn-prev").addEventListener('click', () => {
            ...
            if (this.currentPage == 0) {
                ...
                this.initPagination();
                renderContent(this.currentPage);
            }
        })
