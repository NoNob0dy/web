...
async function renderChart() {
    ...
    charData.xAxis.data = [];
    charData.series[0].data = [];
    charData.series[1].data = [];
    for (key in result.data.saleObj) {
        charData.xAxis.data.push(key);
        charData.series[0].data.push(result.data.saleObj[key]);
    }
    for (key in result.data.countObj) {
        charData.series[1].data.push(result.data.countObj[key]);
    }
